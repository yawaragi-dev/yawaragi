...
- A new env var `MCP_SAKENOWA_URL` (or similar — naming finalised in S1, #139) holds the production MCP-server URL. Set on Vercel (Production + Preview).
- Add a CI `env:` entry with a dummy URL value so the e2e webserver still boots, mirroring the pattern for `CRON_SECRET` and `ANTHROPIC_API_KEY` in `.github/workflows/ci.yml`.
