...
A self-declared 18+ confirmation modal is shown on first visit and persisted in a cookie with a 1-year expiry. No flavor data, brand pages, recommendations, or label scans render before the user accepts. The gate lives in `src/components/legal/AgeGate.tsx` and is enforced by Next.js middleware that blocks every route except `/`, `/about`, `/privacy`, and `/imprint` until the cookie is set. The cookie banner (GDPR) is a separate component from the age gate (JMStV) — they answer different legal questions and must not be conflated.
