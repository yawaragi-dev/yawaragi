...
- **The public example is a flavor surface and inherits the full compliance stack** — it renders behind the 18+ age gate, and any Sakenowa flavor data carries `<SakenowaAttribution />`, any cross-beverage lookup carries `<HeuristicDisclaimer />`, and derived/LLM values carry `<ProvenanceBadge />`. Reframing `/profile` into an "example" must not drop these (the shipped surface already carries them). Compliance follows the data, not the auth state: an *unauthenticated example* has the same obligations as the authenticated journal.
