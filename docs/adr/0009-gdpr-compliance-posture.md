...
### Phase 5 taste events (Upstash-backed; live with #231)

| Store | Data | Lawful basis | Retention | Notes |
|---|---|---|---|---|
| `taste:<sid>` (Upstash Redis list) | per-session list of TasteEvents — each `{kind, brandId or descriptor, 6-axis flavor target, occurredAt}`; keyed to the opaque `yawaragi_session.sid`, **no account/Clerk identifier** | consent (personalisation, Art. 6(1)(a)) | 24h from last write (TTL refreshed on every append; matches the session cookie) | Phase 5 / #220 ([ADR-0019](./0019-taste-profile-derived-from-session-events.md)). The TasteProfile is **derived** from these on read, never stored as a snapshot. Erasure = drop the session key. Pseudonymous, not Art. 9. Debug traces of this data are gated behind `yawaragi_debug` and are in scope for #120's pre-launch leakage audit. |
| `journal:user:<clerkUserId>` (Upstash Redis hash) | per-user JournalEntries — each `{event (kind, brandId, 6-axis target, occurredAt), denormalised sake name, notes, triedAt, createdAt}`; keyed to the **Clerk user id** — this is **account-linked personal data**, unlike the pseudonymous `taste:<sid>` row above | consent (personalisation, Art. 6(1)(a)) | **no expiry** — a journal is a permanent record; erased on request (drop the user key, or one field for a single entry) | Phase 5.5 / #244 ([ADR-0020](./0020-tasting-journal-spine-and-maintainer-only-persistence.md)). v1 is **maintainer-only** (private beta) — the sole data subject is the maintainer. The TasteMap is **derived** from these on read, never stored. Portability via `pnpm journal:export` (shipped, P5.5-D) — a verbatim JSON export of the user key, which doubles as the durability backstop for an Upstash-of-record store. Not Art. 9 (`notes` is free-text tasting notes only). |
...
| **Upstash** | processor | (1) rate-limit budget store: salted SHA-256 of visitor IP + opaque `yawaragi_session.sid` — both keyed only (never plaintext); no account identifiers. (2) Phase 5 / #220: per-session TasteEvent lists keyed by the same `yawaragi_session.sid` — see the taste-events processing row above. (3) Phase 5.5 / #244: per-user JournalEntry hashes keyed by the **Clerk user id** — **account-linked**, no TTL — see the journal processing row above. | EU region (e.g. `eu-central-1` Frankfurt) — selected at database provisioning time | EU SCCs (data does not leave the EU region) | (1)+(2) 24h TTL; (3) no expiry (permanent journal, erased on request) | <https://upstash.com/trust> — **DPA: pending signature before Production deployment; must cover the account-linked journal data (3) before the private beta widens beyond the maintainer** (see consequences below) |
