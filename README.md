...
_Snapshot generated 2026-08-18 from GitHub Issues + merged PRs. Regenerate with `pnpm progress`. Detail: [docs/PROGRESS.md](./docs/PROGRESS.md)._

| Milestone | Progress | Issues | ETA (median) |
| --- | --- | --- | --- |
| **M1 (Phase 0) — Compliance & i18n foundation** | `████████████████████` 100% | 5 / 5 | done |
| **M2 (Phase 2) — Data foundation** | `███████████████████░` 94% | 11 / 12 | 2026-08-24 |
| **M3 (Phases 3–5) — Flagship surfaces** | `██████████████████░░` 91% | 10 / 11 | 2026-08-24 |
...
flowchart TD
  ...
  LL --> SignIn["[locale]/sign-in<br/>(maintainer login, no public sign-up<br/>Phase 5.5, ADR-0020)"]
  SignIn --> Clerk["Clerk<br/>(session → auth().userId)"]
  LL --> Profile["[locale]/profile<br/>(taste map, Phase 5, live)"]
  Profile --> TasteRead["lib/taste/<br/>read-session-taste-profile<br/>+ taste-recommender"]
  TasteRead --> KV[("Upstash Redis<br/>taste:&lt;sid&gt; 24h TTL")]
  Profile -->|"maintainer only"| Journal["components/profile/journal<br/>(map hero + timeline, Phase 5.5)"]
  Clerk --> Gate["lib/auth/maintainer<br/>(allowlist, fail-closed)"]
  Gate --> Journal
  Journal --> JournalStore["lib/taste/journal-store<br/>(port: in-memory | Upstash)"]
  JournalStore --> JKV[("Upstash Redis<br/>journal:user:&lt;id&gt; no TTL")]
  Export["pnpm journal:export<br/>(GDPR portability +<br/>durability backstop)"] --> JournalStore
...
Phase 0 (i18n + legal scaffolding + EN-first launch) and Phase 2 (data foundation, Sakenowa mirror, scheduled cron ingest, provenance schemas) are shipped. **Phase 3 (anonymous label scan)** is in flight — see the milestone bar above for the live status. The entry page + canvas downscale + Sakenowa lookup chain (S1), the anonymous-session rate limit (S2), the Anthropic vision provider (S3), the three-tier confidence UX + two-tier vision retry + manual-curation layer (S4 PR A), the in-place result card on `/scan` (UX-B, ADR-0015 — a confident match renders photo + kanji + flavor chart + deep-link in place, replacing the earlier auto-redirect), and the reverse cross-beverage exemplar layer on the scan card (UX-C — "interesting for those who like Lagavulin 16" via a 62-row hand-curated exemplar table + deterministic L2 lookup, with a `no-close-analog` sentinel for sakes that legitimately have no Western analog) are all live. S4 PR B (enriched disambiguation list, no-match enrichment, the "Not this one?" affordance, full Playwright matrix) is live; the label-scan eval harness (S5, `pnpm eval label-scan-jp`) is built and awaits its rights-cleared photo corpus. The UX redesign (#168) landed on top: a **landing hero that leads with a real example scan result** (UX-E — the maintainer's own 木戸泉 bottle + its real Sakenowa flavor chart + the reverse cross-beverage hook, shown only after the 18+ gate), and the **shared scan result card restyled as an elevated "bone" card** (UX-F) used by both the scan flow and that hero. Two-tier vision means Haiku 4.5 runs every scan; Sonnet 4.6 retries on any tier-1 result other than a clean first-pass match, recovering the calligraphic-kanji and field-swap cases at ~6× cost only on those bottles. The manual-curation layer (ADR-0014) lets the maintainer add brands missing from Sakenowa's public data (`pnpm add-manual-brand` — UMAMI is the seed). Note: the Sakenowa API is live and maintained into 2026 — the earlier "frozen 2024 dump" assumption was disproven; the real gap is sparse 6-axis flavor-chart coverage for new/imported bottles, addressed by a live re-sync + honest "coming soon" UX + a small curated EU/US delta ([ADR-0016](./docs/adr/0016-data-strategy-sakenowa-freshness-and-curated-delta.md)). **Phase 4 (single-shot suggestions over MCP + cross-beverage)** is shipped: the `/[locale]/suggest` surface takes a `?seed=<brandId>` from the sake detail page's "Find similar" link OR a freeform text query, runs a single AI SDK tool loop with the Sakenowa MCP tools + the deterministic `mapCrossBeverage` cross-beverage bridge, and returns a per-field-provenance card list with inline `<HeuristicDisclaimer />` on cross-beverage results. Every call is Langfuse-traced; the `evals/suggest-jp/` harness (`pnpm eval suggest-jp`) runs 15 seed queries with ground-truth recall@k. See PRD [#138](https://github.com/yawaragi-dev/yawaragi/issues/138). **Phase 5 (taste profile)** is largely shipped as an anonymous, **session-scoped v1** ([ADR-0019](./docs/adr/0019-taste-profile-derived-from-session-events.md)): a derived six-axis taste vector — fed by sake ratings, scan-accepts, and cross-beverage seeds — rendered as a flavor radar on `/[locale]/profile` with a similarity-based "Recommended for you" list. The vector is *derived from* stored TasteEvents (session-keyed in Upstash), never a stored snapshot; **account-based** persistence (a `user_taste_vectors` table) is deferred with auth resumption. Still open in the phase: recommender eval + the phase-close architecture pass. **Phase 5.5** (a tasting journal as the spine, IMDb-Bayesian rating display, context-weighted updates) is largely shipped per [ADR-0020](./docs/adr/0020-tasting-journal-spine-and-maintainer-only-persistence.md): a `JournalStore` port with in-memory + Upstash adapters, a Clerk maintainer allowlist, maintainer-gated write actions, a deterministic sake picker, the map-hero + timeline journal UI on `/[locale]/profile`, a maintainer sign-in surface, and `pnpm journal:export` for GDPR portability. Persistence is deliberately **maintainer-only** in v1 (private beta, no public sign-up); the public keeps the interactive-but-ephemeral example. Still open: per-entry edit/delete UI (C2c-ii) and the phase-close architecture pass. Scoped originally per the journal/ratings/insights research ([`docs/research/journal-ratings-insights-feature-design.md`](./docs/research/journal-ratings-insights-feature-design.md)). Every Phase 3+ surface is designed to survive being wrapped in a native webview shell per [ADR-0012](./docs/adr/0012-webview-able-architecture.md).
...
| `pnpm journal:export`     | Export a maintainer's tasting journal to JSON (GDPR portability + durability backstop). **Output holds personal data** — gitignored; delete when done |
